...
export default function FBMockup() {
  ...
  const connectedFB = pages.filter(
    (p) => p.platform === "facebook" && p.isConnected,
  );
  ...
  return (
    <div className="fb-mockup fade-in">
      <div className="fb-mockup-header">
        <img
          src={
            connectedFB.length > 0 ? connectedFB[0].avatar : currentUser.avatar
          }
          className="fb-avatar"
          alt=""
        />
        <div className="fb-header-info">
          <span className="fb-page-name">
            {connectedFB.length > 0 ? connectedFB[0].name : "SAPMA Hub Halaman"}
          </span>
      ...
      <div className="fb-mockup-caption">
        {postCaption ||
          "Tulis caption di sebelah kiri untuk melihat pembaruan instan..."}
      </div>
